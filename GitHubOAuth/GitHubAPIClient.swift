...
enum GitHubRequestType {
    ...
    case oauth //redirects github access
    case token(url: URL)
...
struct GitHubAPIClient {
    ...
    private static func generateURLRequest(_ type: GitHubRequestType) -> URLRequest? {
        ...
        case .token(url: let url):
            ...
            guard let unwrappedCode = code else { return nil }
            let parameters = type.buildParams(with: unwrappedCode)
            ...
            guard let unwrappedParameters = parameters else { return nil }
            
            if JSONSerialization.isValidJSONObject(unwrappedParameters) {
              request.httpBody = try! JSONSerialization.data(withJSONObject: unwrappedParameters, options: [])
            }
    ...
    private static func processToken(response: (Data?, URLResponse?, Error?)) -> Response {
        ...
        guard let unwrappedData = data else { return (nil, nil, GitHubError.data) }
        
        do {
            let serializedData = try JSONSerialization.jsonObject(with: unwrappedData, options: []) as! [String:Any]
            let accessToken = serializedData["access_token"]
            guard  let token = accessToken else { return (nil, nil, GitHubError.data) }
            let error = saveAccess(token: token as! String)
            return (nil, nil, error)
        } catch {
